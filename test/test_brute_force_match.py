from brute_force_match import match
from random import randint

def random_dense_array(width):
    return [[randint(0, (1 << 32) - 1) for _ in range(width)] for _ in range(width)]

def random_sparse_array(width):
    result = [[0] * width for _ in range(width)]
    result[randint(0, width - 1)][randint(0, width - 1)] = randint(0, (1 << 32) - 1)
    return result
